...
public final class StreamingDataAnalyzer {
	...
	public static void main(String[] args) throws Exception {
		...
		// "ip-172-31-2-73:9092,ip-172-31-2-70:9092,ip-172-31-2-77:9092,ip-172-31-2-75:9092");
		...
		SparkSession spark = JavaSparkSessionSingleton.getInstance(conf,
				props.getProperty("spark.cassandra.connection.host"));
		// load the experts from Cassandra
		Dataset<Row> expertsDF = spark.read().format("org.apache.spark.sql.cassandra")
		...
		// parse the input stream and create Post object (Function<Tuple2<arg1,
		// arg2>, returnType>)
		...
		postStream.foreachRDD(new VoidFunction2<JavaRDD<StreamPost>, Time>() {
			...
			public void call(JavaRDD<StreamPost> rdd, Time time) {
				...
				// Store incoming Post in the live_posts_by_day table in
				// cassandra with group_day as partition key
				storeResults(spark, postDF, "POSTS",
		...
		JavaPairDStream<String, Integer> streamTags = questions
				.mapToPair(new PairFunction<StreamPost, String, Integer>() {
					@Override
		...
		// - Window Operations
		JavaPairDStream<String, Integer> windowTagCount = streamTags.reduceByKeyAndWindow(reduceTagFunc,
				invReduceTagFunc, Durations.seconds(windowDuration), Durations.seconds(slidingDuration));
		...
		questions.foreachRDD(new VoidFunction2<JavaRDD<StreamPost>, Time>() {
			...
			public void call(JavaRDD<StreamPost> rdd, Time time) {
				...
				Dataset<Row> post_experts = storeResults(spark, questions, "QUESTIONS",
				...
				// Storing last group_hour processed for posts_experts - need
				// for querying data
				storeResults(spark, post_experts, "POSTS_EXPERTS",
	 ...
	 */
	static Dataset<Row> storeResults(SparkSession spark, Dataset<Row> tagDF, String type, String query,
			final String table) {
	...
	static Dataset<Row> storeResults(SparkSession spark, Dataset<Row> tagDF, final String table, SaveMode savemode) {
		// Save to database
		tagDF.write().format("org.apache.spark.sql.cassandra").options(new HashMap<String, String>() {
			{
				put("keyspace", "stackoverflow");
				put("table", table);
			}
