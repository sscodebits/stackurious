...
def main():
    ...
    if len(sys.argv) != 6:
        print "Usage: ./create_streaming_data.py #topicName #tagfile #numminutes #numentries #startid "
        sys.exit(1)
    ...
    for repeat in range(0, n_repeats):
      ...
      for line in range(0, n_lines):
        ...
        # write line to file
        produceMsg(topic, ",".join(map(str, [id,postTypeId,parentId,creationDate,tags,title,acceptedAnswer]))  )
      #pause between Set
      sleep(n_pauseTime)
